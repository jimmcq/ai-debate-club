...
import { getPersonaConfig, getAllPersonaTypes, getRandomPersonaType, getPersonaEntries, PersonaType } from '@/lib/personas'
...
describe('personas', () => {
  ...
  describe('getPersonaConfig', () => {
    validPersonaTypes.forEach(personaType => {
      describe(`${personaType} persona`, () => {
        ...
        it('has comprehensive system prompt', () => {
          ...
          expect(config.systemPrompt.toLowerCase()).toMatch(/(debate|argument|position)/i)
        })
    ...
    it('throws error for undefined persona type', () => {
      expect(() => {
        getPersonaConfig(undefined as any)
      }).toThrow()
    })

    it('throws error for null persona type', () => {
      expect(() => {
        getPersonaConfig(null as any)
      }).toThrow()
    })
  })
  ...
  describe('persona characteristics', () => {
    ...
    it('showman persona emphasizes performance and engagement', () => {
      ...
      expect(config.systemPrompt.toLowerCase()).toMatch(/(dramatic|engaging|performance|charisma)/i)
    })

    it('contrarian persona emphasizes opposing views', () => {
      const config = getPersonaConfig('contrarian')

      expect(config.systemPrompt.toLowerCase()).toMatch(/(contrary|oppose|alternative|challenge)/i)
    })

    it('diplomat persona emphasizes balance and nuance', () => {
      const config = getPersonaConfig('diplomat')

      expect(config.systemPrompt.toLowerCase()).toMatch(/(balanced|nuanced|diplomatic|moderate)/i)
    })

    it('philosopher persona emphasizes deep thinking', () => {
      const config = getPersonaConfig('philosopher')

      expect(config.systemPrompt.toLowerCase()).toMatch(/(philosophical|profound|deeper|meaning)/i)
    })

    it('scientist persona emphasizes evidence and methodology', () => {
      const config = getPersonaConfig('scientist')

      expect(config.systemPrompt.toLowerCase()).toMatch(/(evidence|data|research|scientific)/i)
    })

...
import { createDebate, getDebatePhase, getCurrentTurnNumber, canAddCurveball, isDebateComplete, getNextParticipant } from '@/lib/debate/utils'
  ...
  describe('End-to-End Debate Simulation', () => {
    it('executes a complete debate flow from creation to completion', () => {
      ...
      for (let turn = 1; turn <= 8; turn++) {
        ...
        const personaType = currentDebate.participants.find(p => p.id === participantId)?.personaType
        const personaConfig = getPersonaConfig(personaType!)
    ...
    it('handles curveball integration during rebuttal phase', () => {
      ...
      expect(lastMessage.curveball).toBe('What about the environmental impact of AI computation?')
  ...
  describe('Data Consistency and State Management', () => {
    it('maintains consistent state throughout debate progression', () => {
      ...
      stateHistory.forEach((state, index) => {
        ...
        if (index > 0) {
          expect(state.currentTurn).toBeGreaterThanOrEqual(stateHistory[index - 1].currentTurn)
        }
      })
    ...
    it('handles edge cases in state transitions', () => {
      const debate = createDebate('Edge case topic?', 'diplomat', 'showman')
